...
# ============================================================
# Unavailable losses — verified by diffbmp, documented for reference
# ============================================================

def ssim_loss(_rendered, _target):
    """
    ❌ UNAVAILABLE — DO NOT USE.

    Verified by diffbmp (loss_functions.py L155-180):
    SSIM loss conflicts with gradient descent optimization.
    In tests, loss increased by +21.7% instead of decreasing.

    Reason: SSIM is a perceptual metric designed for evaluating image quality,
    not as a differentiable loss for gradient-based optimization of
    individual shapes. Its gradient landscape has local maxima that
    trap the optimizer.

    Reference: diffbmp/pydiffbmp/util/loss_functions.py — NotImplementedError
    """
    raise NotImplementedError(
        "SSIM loss is incompatible with gradient-based vectorization. "
        "Verified by diffbmp: loss diverges (+21.7%). "
        "Use perceptual loss (VGG-based) instead for structure-aware optimization."
    )


def edge_loss(_rendered, _target):
    """
    ❌ UNAVAILABLE — DO NOT USE.

    Verified by diffbmp (loss_functions.py L155-180):
    Edge loss (Sobel-based) gradients conflict with other loss components
    (MSE, perceptual), causing the optimizer to oscillate.

    Reason: Edge maps change rapidly as shapes move, creating a
    non-smooth loss landscape that destabilizes Adam.

    Reference: diffbmp/pydiffbmp/util/loss_functions.py — NotImplementedError
    """
    raise NotImplementedError(
        "Edge loss (Sobel) is incompatible with gradient-based vectorization. "
        "Verified by diffbmp: gradients conflict with other losses. "
        "Use perceptual loss (VGG-based) instead for edge/structure awareness."
    )
